...
void stronglyComponentCount(List listInput[], int n)
{
	...
	for (int i = 0; i < n; ++i)
	{
		current = listInput[i].head;
		while (!current == NULL)
		{
			addToList(list, current->number - 1, i + 1);
			current = current->next;
		}
	}
	...
	while (!current == NULL)
	{
		if (!visited[current->number - 1])
		{
			explore(listInput, visited, current->number);
			++counter;
		}
...
int main()
{
	...
	stronglyComponentCount(nodes, n);
